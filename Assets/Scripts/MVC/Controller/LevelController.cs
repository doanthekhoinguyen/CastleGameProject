    ...
    namespace MVC.Controller
    {
        ...
        public class LevelController : MonoBehaviour
        {
            ...
            public int GetInitialCoinsForLevel(int levelNumber)
            {
                var levelSpawnConfig = FindLevelSpawnConfig(levelNumber);
                if (levelSpawnConfig != null)
                {
                    return levelSpawnConfig.initialCoin;
                }
                else
                {
                    Debug.LogError($"Cannot find spawn configuration for level {levelNumber}");
                    return 0; // Trả về 0 nếu không tìm thấy config
                }
            }
